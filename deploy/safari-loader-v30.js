(() => {
  'use strict';
  if (window.__SAFARI_LOADER_V30_READY__) return;
  window.__SAFARI_LOADER_V30_READY__ = true;
  ...
  function loadArchiveFix() {
    ...
    archivePromise = loadScript('./deploy/v29-archivo.js?v=30', 'safari-archivo-v30-js')
      .then(() => {
        if (!window.__SAFARI_ARCHIVO_V29_READY__) {
          throw new Error('El módulo Carpeta no terminó de iniciar');
        }
      })
      .catch((error) => {
  ...
  function loadComodinLocation() {
    ...
    comodinLocationPromise = loadScript('./deploy/v30-comodines-location.js?v=30', 'safari-comodines-location-v30-js')
      .then(() => {
        if (!window.__SAFARI_COMODINES_LOCATION_V30_READY__) {
          throw new Error('El módulo de ubicación de comodines no terminó de iniciar');
        }
      })
      .catch((error) => {
        console.error('Safari Comodines V30 no pudo cargarse:', error);
  ...
  async function ensureLeaflet() {
    addStylesheet('https://unpkg.com/leaflet@1.9.4/dist/leaflet.css', 'safari-leaflet-v30-css');
    if (window.L?.map) return;
    await loadScript('https://unpkg.com/leaflet@1.9.4/dist/leaflet.js', 'safari-leaflet-v30-js');
    if (!window.L?.map) throw new Error('Leaflet no quedó disponible');
  }

  function loadMeta() {
    if (metaPromise) return metaPromise;
    addStylesheet('./deploy/v23-meta.css?v=30', 'safari-meta-v30-css');
    metaPromise = loadScript('./deploy/v23-meta.js?v=30', 'safari-meta-v30-js')
  ...
  async function loadMapStack() {
    ...
    mapPromise = (async () => {
      ...
      addStylesheet('./deploy/v16-map.css?v=30', 'safari-map-v30-core-css');
      addStylesheet('./deploy/v22-map-mobile-search.css?v=30', 'safari-map-v30-search-css');

      await loadScript('./deploy/v22-map-mobile-search.js?v=30', 'safari-map-v30-search-js');
      await loadScript('./deploy/v27-map.js?v=30', 'safari-map-v30-core-js');
      ...
      console.error('No se pudo iniciar Safari Map V30:', error);
      mapPromise = null;
      throw error;
    });
  ...
  function wireLazyLoading() {
    ...
    if (enterBtn && enterBtn.dataset.v30MetaWire !== '1') {
      enterBtn.dataset.v30MetaWire = '1';
      enterBtn.addEventListener('click', () => {
        setTimeout(loadMeta, 0);
        setTimeout(loadArchiveFix, 0);
        setTimeout(loadComodinLocation, 0);
    ...
    document.addEventListener('click', event => {
      ...
      loadMapStack()
        ...
        .catch(error => {
          restoreButton(originalHTML);
          alert(`No se pudo cargar el mapa V30: ${error?.message || 'error desconocido'}.`);
        });
    }, true);
  }
